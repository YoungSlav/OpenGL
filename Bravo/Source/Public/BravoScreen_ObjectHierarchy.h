...
public:
	...
	BravoScreen_ObjectHierarchy(Args&&... args) :
		BravoScreen(std::forward<Args>(args)...)
	{}


	void SetRoot(std::shared_ptr<class BravoObject> _RootObject) { RootObject = _RootObject; }
...
private:
	std::weak_ptr<class BravoObject> RootObject;
